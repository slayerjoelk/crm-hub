...
export async function ensureTables() {
  try {
    await client.execute(`
      CREATE TABLE IF NOT EXISTS workspaces (id text PRIMARY KEY, slug text UNIQUE NOT NULL, name text NOT NULL, description text, domain text, logo_url text, primary_color text DEFAULT '#2563eb', accent_color text DEFAULT '#0d9488', plan text NOT NULL DEFAULT 'free', status text NOT NULL DEFAULT 'active', stripe_customer_id text, stripe_subscription_id text, billing_email text, created_at integer, updated_at integer);
