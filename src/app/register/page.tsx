    ...
    <div className="min-h-screen bg-[#08090a] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-[#62666d] text-sm mt-1">Set up your CRM</p>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="pt-2 border-t border-white/[0.06]">
            <label className="block text-xs font-medium text-[#8a8f98] mb-1.5">Company / Workspace Name</label>
            <input value={workspaceName} onChange={(e) => { setWorkspaceName(e.target.value); if (!workspaceSlug) setWorkspaceSlug(e.target.value.toLowerCase().replace(/\s+/g, "-")); }} required placeholder="MintAgree"
              className="w-full h-10 px-4 rounded-lg bg-[#0f1011] border border-white/[0.06] text-sm text-[#d0d6e0] placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-emerald-500/50" />
          </div>
          <div>
            <label className="block text-xs font-medium text-[#8a8f98] mb-1.5">Workspace Slug</label>
            <input value={workspaceSlug} onChange={(e) => setWorkspaceSlug(e.target.value.toLowerCase().replace(/\s+/g, "-"))} required placeholder="mintagree"
              className="w-full h-10 px-4 rounded-lg bg-[#0f1011] border border-white/[0.06] text-sm text-[#d0d6e0] placeholder-slate-600" />
          </div>
        ...
        <div className="mt-6 text-center"><a href="/login" className="text-sm text-[#62666d] hover:text-[#10b981] transition-colors">Already have a workspace? Sign in</a></div>
      </div>
    </div>
  );
}
